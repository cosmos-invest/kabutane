const SignalV2 = (() => {
  ...
  const T = { DAILY: "@@SV2_D@@", FAST: "@@SV2_F@@", SLOW: "@@SV2_S@@", RAW: "@@SV2_R@@", CONDITION: "@@SV2_C@@" };
  ...
  function rewriteText(input) {
    ...
    text = text
      .replaceAll("日足RSI14", T.DAILY)
      .replaceAll("月足RSI14 × RSI14の5か月SMA", T.CONDITION)
      .replaceAll("月足RSI14 > RSI14の5か月SMA", T.CONDITION)
      .replaceAll("月足RSI5 > 月足RSI14", T.CONDITION)
      .replaceAll("RSI14・5か月MA", T.SLOW)
      .replaceAll("RSI14の5か月SMA", T.SLOW)
      .replaceAll("RSI14の5か月MA", T.SLOW)
      .replaceAll("Wilder RSI14", `Wilder ${T.RAW}`)
      .replaceAll("ワイルダー方式のRSI14", `ワイルダー方式の${T.RAW}`)
      .replaceAll("TradingView方式のRSI14", `TradingView方式の${T.RAW}`)
      .replaceAll("月足RSI5 / RSI14", `${T.FAST} / ${T.SLOW}`)
      .replaceAll("月足RSI5・RSI14", `${T.FAST}・${T.SLOW}`)
      .replaceAll("月足RSI5", T.FAST)
      .replaceAll("月足RSI14", T.FAST)
      .replaceAll("RSI5がRSI14以下", `${T.FAST}が5か月MA以下`)
      .replaceAll("RSI5がRSI14を上回る", `${T.FAST}が5か月MAを上回る`)
      .replaceAll("RSI5がRSI14", `${T.FAST}が5か月MA`)
      .replaceAll("RSI5≥60・RSI14上向き", `${T.FAST}≥60・5か月MA上向き`)
      .replaceAll("RSI5が60以上", `${T.FAST}が60以上`)
      .replaceAll("RSI14が上向き", "5か月MAが上向き")
      .replaceAll("RSI14上向き", "5か月MA上向き")
      .replaceAll("RSI5 最低値", `${T.FAST} 最低値`)
      .replaceAll("RSI5", T.FAST)
      .replaceAll("RSI14", T.SLOW)
    ...
    return text
      .replaceAll(T.CONDITION, "月足RSI14 > RSI14の5か月SMA")
      .replaceAll(T.FAST, FAST_LABEL)
      .replaceAll(T.SLOW, SLOW_LABEL)
      .replaceAll(T.DAILY, "日足RSI14")
      .replaceAll(T.RAW, "RSI14");
  }
  ...
  function rewriteNode(node) {
    ...
    const element = node;
    if (["SCRIPT", "STYLE", "TEXTAREA"].includes(element.tagName) || element.hasAttribute("data-signal-canonical")) return;
  ...
  function rewriteCharts() {
    ...
    Object.values(window.Chart.instances).forEach((chart) => {
      ...
      (chart.data?.datasets || []).forEach((dataset) => {
        ...
        else if (["RSI5", "月足RSI5"].includes(dataset.label)) next = FAST_LABEL;
        else if (["RSI14", "月足RSI14"].includes(dataset.label)) next = SLOW_LABEL;
        else next = rewriteText(dataset.label);
        if (next !== dataset.label) { dataset.label = next; changed = true; }
      });
  ...
  function injectMethodCard() {
    ...
    section.innerHTML = `<div><span class="signal-method-kicker">OFFICIAL SIGNAL DEFINITION</span><h2>TradingView方式の月足RSI14と、その5か月移動平均で統一</h2><p>完成済み月足の終値からWilder RSI14を計算し、RSI14が5か月SMAを上回る期間を対象とします。</p><div id="signalStaleWarning" class="signal-stale-warning" hidden></div></div><div class="signal-method-actions"><span id="signalVersionBadge" class="signal-version-badge">データ方式を確認中…</span><a class="signal-method-link" href="signal-method.html">計算方法を見る</a></div>`;
  ...
  async function verifyDataVersion() {
    ...
    try {
      ...
      if (payload.signal_version !== VERSION) {
        ...
        if (badge) { badge.textContent = "旧方式データ・更新待ち"; badge.className = "signal-version-badge stale"; }
        if (warning) { warning.hidden = false; warning.textContent = "現在公開中のJSONは旧RSI5対RSI14方式です。新方式の全銘柄再計算が完了するまで、数値・対象銘柄・バックテストを操作できません。"; }
        return false;
      }
      document.body.classList.remove("signal-data-stale");
      if (badge) { badge.textContent = "TradingView方式で統一済み"; badge.className = "signal-version-badge ready"; }
      return true;
    } catch (error) {
      if (badge) { badge.textContent = "データ方式を確認できません"; badge.className = "signal-version-badge error"; }
      if (warning) { warning.hidden = false; warning.textContent = `計算方式の確認に失敗しました: ${error.message}`; }
      return false;
    }
  }
