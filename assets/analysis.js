...
function cacheElements() {
  [
    "generatedAt", "priceBasis", "startMonth", "endMonth", "rsiMin", "rsiMax", "returnMin", "returnMax", "searchInput", "resetFilters",
...
function populateMonths() {
  const months = [...new Set((state.data.episodes || []).map((row) => row.start_month))].sort();
...
function baseRows() {
  ...
  const rsiMin = inputNumber(els.rsiMin);
  const rsiMax = inputNumber(els.rsiMax);
  const returnMin = inputNumber(els.returnMin);
  const returnMax = inputNumber(els.returnMax);
  const query = els.searchInput.value.trim().toLowerCase();

  return (state.data.episodes || []).filter((row) => {
    const strength = finite(row.start_rsi_strength);
    const result = finite(row.return_pct);
    if (start && row.start_month < start) return false;
    if (end && row.start_month > end) return false;
    if (rsiMin !== null && (strength === null || strength < rsiMin)) return false;
    if (rsiMax !== null && (strength === null || strength > rsiMax)) return false;
    if (returnMin !== null && (result === null || result < returnMin)) return false;
    if (returnMax !== null && (result === null || result > returnMax)) return false;
...
function renderRows(rows) {
  ...
  els.tbody.innerHTML = pageRows.map((row) => {
    ...
    return `<tr>
      ...
      <td class="num">${number(row.start_price)}</td><td>${escapeHtml(row.end_month || row.valuation_date || "最新")}</td><td class="num">${number(row.end_price)}</td>
      <td class="num">${number(row.start_rsi14)}</td><td class="num">${number(row.start_rsi5)}</td><td class="num">${number(row.start_rsi_strength)}</td>
      <td class="num">${number(row.duration_months, 0)}</td><td class="num ${performanceClass(row.return_pct)}">${signed(row.return_pct)}</td>
    </tr>`;
  }).join("") || `<tr><td colspan="12" class="empty-state">条件に合う実績がありません。</td></tr>`;
...
function resetFilters() {
  populateMonths();
  [els.rsiMin, els.rsiMax, els.returnMin, els.returnMax, els.searchInput].forEach((element) => { element.value = ""; });
  state.page = 1;
  render();
}

function bindEvents() {
  [els.startMonth, els.endMonth].forEach((element) => element.addEventListener("change", () => { state.page = 1; render(); }));
  [els.rsiMin, els.rsiMax, els.returnMin, els.returnMax, els.searchInput].forEach((element) => element.addEventListener("input", () => { state.page = 1; render(); }));
