...
def build_dividend_history(frame: pd.DataFrame, *, now: datetime | pd.Timestamp | None = None) -> dict[str, Any]:
    """Summarize completed calendar-year dividends without extra network access.

    The input is the same yfinance price frame already downloaded by the core
    pipeline with actions=True. Historical dividends are normalized for later
    stock splits so old per-share amounts remain comparable to the latest share
    unit. The current partial calendar year is intentionally excluded.
    """
    empty = {
        "basis": BASIS,
        "source": SOURCE,
    ...
    splits = pd.to_numeric(work.get("Stock Splits"), errors="coerce").fillna(0.0) if "Stock Splits" in work.columns else pd.Series(0.0, index=work.index)

    split_rows = [(pd.Timestamp(date), float(ratio)) for date, ratio in splits.items() if _finite(ratio) not in (None, 0, 1)]
    ...
    for date, amount in dividends.items():
        ...
        for split_date, ratio in split_rows:
            if split_date > pd.Timestamp(date) and ratio > 0:
                factor *= ratio
        adjusted_events.append((pd.Timestamp(date), float(value) / factor if factor else float(value)))

    current_year = _current_year(now)
    first_date = pd.Timestamp(work.index.min())
    first_full_year = int(first_date.year) if (first_date.month == 1 and first_date.day <= 15) else int(first_date.year) + 1
    last_full_year = current_year - 1
    if first_full_year > last_full_year:
        return empty

    annual_totals = {year: 0.0 for year in range(first_full_year, last_full_year + 1)}
    ...
    if not history or not any((_finite(row["annual_dividend"]) or 0) > 0 for row in history):
        return {**empty, "observation_years": len(history), "history": history[-10:]}
