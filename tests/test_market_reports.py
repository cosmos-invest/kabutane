...
class MarketReportsTest(unittest.TestCase):
    def test_normalize_market_and_sector(self):
        ...
        self.assertIsNone(reports.parse_sector_code(""))
    ...
    def test_build_ranking_keeps_previous_position_and_price(self):
        ...
        previous = {"price_date": "2026-07-23", "rows": [
        ...
        self.assertEqual(ranking["comparison_price_date"], "2026-07-23")
        self.assertEqual(latest["records"][1]["gc_return_rank"], 1)

    def test_same_day_rerun_uses_original_comparison_snapshot(self):
        previous = {
            "price_date": "2026-07-24",
            "comparison_price_date": "2026-07-23",
            "comparison_rows": [{"code": "1111", "rank": 3, "current_price": 100}],
            "rows": [{"code": "1111", "rank": 1, "current_price": 120}],
        }
        latest = {"records": [{"code": "1111", "current_price": 120, "return_since_gc_pct": 20}]}
        ranking = reports.build_ranking(latest, previous, "2026-07-24")
        self.assertEqual(ranking["rows"][0]["previous_rank"], 3)
        self.assertEqual(ranking["rows"][0]["rank_change"], 2)
        self.assertEqual(ranking["rows"][0]["daily_change_pct"], 20)
